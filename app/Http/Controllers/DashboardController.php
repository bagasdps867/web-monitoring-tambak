...
class DashboardController extends Controller
{
    ...
    /**
     * Endpoint khusus polling ringan per 5 detik untuk real-time alat
     */
    public function getLatestSensor()
    {
        ...
        if ($latest->kualitas === null) {
            $latest->kualitas = round($this->calculateFuzzyQuality(
                $latest->ph ?? 7.0,
                $latest->suhu ?? 28.0,
                $latest->tds ?? 0.0,
                $latest->kekeruhan ?? 0.0
            ), 2);
        }
    ...
    public function getSensorData()
    {
        ...
        if ($latest) {
            $quality = round($this->calculateFuzzyQuality(
                $latest->ph ?? 7.0,
                $latest->suhu ?? 28.0,
                $latest->tds ?? 0.0,
                $latest->kekeruhan ?? 0.0
    ...
    /**
     * Endpoint utama untuk mengambil analisis Hybrid AI (ML + Fuzzy) & Tindakan SOP
     */
    public function getAiRecommendation()
    {
        ...
        // 1. Panggil Service Flask Python Hybrid AI
        try {
        ...
        // 2. Fallback Lokal jika Flask Python sedang mati
        if ($isOffline) {
            $score = $this->calculateFuzzyQuality($ph, $suhu, $tds, $kekeruhan);
            ...
            $statusPrediksi = 'Offline (Fallback Lokal)';
            $finalStatus    = $statusSaatIni;
            $tindakanSop    = $this->getLocalSopActions($finalStatus);
        }

        // 3. Rekomendasi Detail berbasis Parameter Spesifik
        $emergencyAdvice = $this->getLocalEmergencyAdvice($latest);
        ...
        return response()->json([
            ...
            'tindakan_sop'  => $tindakanSop,
            'advice'        => $emergencyAdvice
        ]);
    }

    /**
     * Tindakan Cadangan SOP lokal berdasarkan status (Jika AI Offline)
     */
    private function getLocalSopActions($status)
    {
        if ($status === 'Baik') {
            return [
                'Budidaya dilanjutkan secara normal.',
                'Pemberian pakan dilakukan sesuai jadwal.',
                'Monitoring IoT tetap berjalan secara konstan.'
            ];
        } elseif ($status === 'Sedang') {
            return [
                'Lakukan pemeriksaan kondisi fisik tambak dan sensor.',
                'Amati aktivitas udang/ikan dan respons terhadap pakan.',
                'Tingkatkan frekuensi pengamatan dan hindari pakan berlebihan.'
            ];
        } else {
            return [
                'Lakukan verifikasi pembacaan sensor dengan alat pembanding.',
                'Evaluasi pemberian pakan dan sirkulasi air tambak.',
                'Lakukan tindakan korektif pengelolaan/pergantian air secara bertahap.'
            ];
        }
    }
